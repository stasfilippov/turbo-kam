			...
			<div className='w-[540px] h-[260px] relative '>
				...
				<div className='absolute z-20 top-4 left-4 flex flex-col items-start gap-4'>
					{typesCar.map((car, index) => {
						return <Badge className='py-[2px] px-2 bg-primary-pink rounded-5xl medium-16 text-primary-white' key={index}>{car.slice(0,1).toUpperCase() + car.slice(1) + ' авто'}</Badge>
					})}
				</div>
			</div>
		</Card>
